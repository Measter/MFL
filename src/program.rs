...
use crate::{
    interners::Interners,
    lexer::{self, Token},
    opcode::{self, Op, OpCode, Procedure},
    simulate::simulate_execute_program,
    source_file::{FileId, SourceStorage},
    type_check, OPT_OPCODE,
};

type LoadProgramResult = Result<Program, Vec<Diagnostic<FileId>>>;

pub struct Program {
    pub global: Procedure,
    pub macros: HashMap<Spur, Procedure>,
    pub procedures: HashMap<Spur, Procedure>,
    pub const_values: HashMap<Spur, Procedure>,
    pub included_files: HashMap<Spur, Vec<Op>>,
    pub include_queue: Vec<(Token, Spur)>,
}
